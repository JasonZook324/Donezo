...
public record ItemRecord(int Id, int ListId, string Name, bool IsCompleted, int? ParentItemId, bool HasChildren, int ChildrenCount, int IncompleteChildrenCount, int Level, string SortKey, int Order, int? CompletedByUserId, string? CompletedByUsername, DateTime? CompletedAtUtc);
...
public partial class NeonDbService : INeonDbService
{
    ...
    private async Task EnsureSchemaAsync(CancellationToken ct)
    {
        ...
        // Always attempt to add new columns introduced after initial schema.
        await using (var addCompletedAtCol = new NpgsqlCommand("alter table if exists items add column if not exists completed_at timestamptz", conn))
        { await addCompletedAtCol.ExecuteNonQueryAsync(ct); }
        _schemaEnsured = true;
    }
    ...
    public async Task<(bool Ok,long NewRevision,int Affected)> ResetSubtreeAsync(int rootItemId, long expectedRevision, CancellationToken ct = default)
    {
        ...
        {
            var sql = "with recursive sub as ( select id,parent_item_id from items where id=@root union all select i.id,i.parent_item_id from items i join sub s on i.parent_item_id=s.id ) update items set is_completed=false, completed_by_user_id=null, completed_at=null where id in (select id from sub);";
    ...
    private async Task<bool> SetItemCompletedByUserInternalAsync(int itemId, int userId, bool completed, CancellationToken ct)
    {
        ...
        if (completed)
        {
            ...
            await using (var upd = new NpgsqlCommand("update items set is_completed=true,completed_by_user_id=@uid,completed_at=now() where id=@i", conn, tx)) { upd.Parameters.AddWithValue("uid", userId == 0 ? (object)DBNull.Value : userId); upd.Parameters.AddWithValue("i", itemId); await upd.ExecuteNonQueryAsync(ct); }
            if (parentId != null)
            {
                await using (var allDone = new NpgsqlCommand("select count(*) from items where parent_item_id=@p and is_completed=false", conn, tx)) { allDone.Parameters.AddWithValue("p", parentId.Value); var remainObj = await allDone.ExecuteScalarAsync(ct); var remain = remainObj is long l2 ? (int)l2 : remainObj is int i2 ? i2 : 0; if (remain == 0) { await using var updParent = new NpgsqlCommand("update items set is_completed=true,completed_by_user_id=@uid,completed_at=now() where id=@p", conn, tx); updParent.Parameters.AddWithValue("uid", userId == 0 ? (object)DBNull.Value : userId); updParent.Parameters.AddWithValue("p", parentId.Value); await updParent.ExecuteNonQueryAsync(ct); } }
            }
        }
        else
        {
            await using (var upd = new NpgsqlCommand("update items set is_completed=false,completed_by_user_id=null,completed_at=null where id=@i", conn, tx)) { upd.Parameters.AddWithValue("i", itemId); await upd.ExecuteNonQueryAsync(ct); }
